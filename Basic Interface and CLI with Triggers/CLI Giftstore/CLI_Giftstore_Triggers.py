...
def remove_from_cart(customer_id):
    try:
        ...
        query = "SELECT * FROM CART WHERE CustomerID = %s AND ProductID = %s"
...
def remove_manager():
    try:
        ...
        cursor.execute(query, (manager_id,))
        manager = cursor.fetchone()
        cursor.execute("Select ManagerId from Manager where ManagerId <> %s",(manager_id,))
        all_mangId = cursor.fetchall()
       
        if manager:
            if len(all_mangId) > 0:
                cursor.execute("Update Employee set ManagerId = %s where ManagerId = %s " , (all_mangId[0][0], manager_id , ))
            else:
