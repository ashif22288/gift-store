...
-- 1.
-- The total number of employees managed by each manager:
SELECT m.ManagerID, m.FirstName, m.LastName, COUNT(e.EmployeeID) AS TotalEmployees
FROM Manager as  m
LEFT JOIN Employee as e ON m.ManagerID = e.ManagerID
GROUP BY m.ManagerID;



-- 2.
-- Selecting those persons who are both vendor and customer
SELECT p.*
FROM person p
INNER JOIN vendor v ON p.person_id = v.person_id
INNER JOIN customer c ON p.person_id = c.person_id;



-- 3.
-- Arrange the  customers in the descreasing order of their total payment amount:
SELECT c.customer_id, SUM(p.Amount) AS TotalPayment
FROM payment as p
INNER JOIN Orders as o ON p.OrderID = o.OrderID
INNER JOIN customer as c ON o.CustomerID = c.customer_id
...
-- 4.
-- number of pending orders for each branch:
Select b.BranchID ,COUNT(o.OrderID) AS PendingOrders
FROM branch as b
LEFT JOIN manager as m ON m.BranchID = b.BranchID
LEFT JOIN employee as e ON m.ManagerID = e.ManagerID
left join orderids as oi ON e.EmployeeID=oi.EmployeeID
left join Orders as o ON o.OrderID=oi.orderids
WHERE o.Status_ = 'Pending'
GROUP BY b.BranchID;
...
-- 5. 
--  product details along with the corresponding reviews for products with a rating greater than 4:
SELECT p.* , r.*
FROM Product as p
INNER JOIN Review as r ON p.ProductID = r.ProductID
WHERE p.Stars > 4;
...
-- 6.
-- Calculating Average of each product type
SELECT p.ProductType, AVG(v.Rating) AS AverageRating
FROM Product p
JOIN vendor v ON p.vendor_id = v.vendor_id
JOIN VendorHistory vh ON p.ProductID = vh.ProductID
...
-- 7.
-- Updating Branch Contact number whose Manager ID given
UPDATE Branch
SET BranchContact = '9876543210'
WHERE BranchID = (
    SELECT BranchID
    FROM Manager
    WHERE ManagerID = 1
...
-- 8. 
-- Updating Price of the Product increased by 10%
UPDATE Product p
JOIN VendorHistory vh ON p.ProductID = vh.ProductID
JOIN Vendor v ON vh.VendorID = v.vendor_id
SET p.ProductPrice = p.ProductPrice * 1.1; -- Increase price by 10%




-- 9.
-- Deleting rows that have been delivired
DELETE o, d
FROM Orders o
JOIN Delivery d ON o.OrderID = d.OrderID
WHERE o.Status_ = 'Delivered';




-- 10.
-- Delete all orders placed by customers who have not placed any orders in the past Month:
DELETE Orders
FROM Orders
JOIN Customer c ON Orders.CustomerID = c.customer_id
